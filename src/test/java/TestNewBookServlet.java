...
import com.my.library.servlets.NewBookCommand;
...
public class TestNewBookServlet extends Mockito {


    @ParameterizedTest
    @CsvFileSource(resources = "test_new_book.csv", numLinesToSkip = 1)
    public void testNewBook(String ISBN, String title, String genreId, String genre,
                            ...
                            String quantity, String caseNum, String shelfNum, String cellNum, String fileName)
        ...
        when(context.getRealPath("/")).thenReturn(Paths.get(".").toAbsolutePath().normalize().toString()+"\\src\\main\\webapp");
        ...
        String resultPage  =new NewBookCommand().execute(request, response, appContext);
        Assertions.assertTrue(resultPage.contains(
                ConfigurationManager.getInstance().getProperty(ConfigurationManager.NEW_BOOK_PAGE_PATH)));
        }
